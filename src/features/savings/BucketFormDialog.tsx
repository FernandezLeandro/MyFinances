...
export function BucketFormDialog({ open, onClose, bucket }: BucketFormDialogProps) {
  ...
  const {
    ...
    defaultValues: { name: '', singleCurrency: false },
  })
  ...
  useEffect(() => {
    if (!open) return
    reset(bucket ? { name: bucket.name, singleCurrency: bucket.single_currency } : { name: '', singleCurrency: false })
  }, [open, bucket, reset])

  async function onSubmit(values: FormValues) {
    if (isEditing) {
      await updateBucket.mutateAsync({ id: bucket.id, name: values.name.trim(), singleCurrency: values.singleCurrency })
    } else {
      await createBucket.mutateAsync({ name: values.name.trim(), singleCurrency: values.singleCurrency })
    }
    onClose()
  }
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5" noValidate>
        ...
        <Field label="Monedas" hint="Restringir a la moneda principal, o permitir ARS y USD">
